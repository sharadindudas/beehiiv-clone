...
export const SegmentFilterSchema = v.optional(
    v.object({
...
// Segment inner condition schema
export const SegmentInnerConditionSchema = v.object({
...
export type SegmentInnerConditionSchema = v.InferInput<typeof SegmentInnerConditionSchema>;

// Segment condition schema (normal or nested)
export const SegmentConditionSchema = v.object({
    ...
    data: v.object({
        name: v.string(),
        type: v.string(),
        logical_operator: v.optional(v.pipe(v.string())),
        filters: SegmentFilterSchema,
        conditions: v.optional(v.pipe(v.array(SegmentInnerConditionSchema)))
    })
});
...
export const SegmentConditionsSchema = v.object({
    conditions: v.object({
        ...
        logical_operator: v.pipe(v.string(), v.nonEmpty("Logical operator is required"))
    })
});
